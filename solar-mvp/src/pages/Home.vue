...
const addressInput = ref('Ho Chi Minh City'); // Default value for easier testing
...
const selectSuggestion = (suggestion: any) => {
    ...
    suggestions.value = [];
    // Optional: Auto-start map or just fill input?
    // User flow implies selecting then going, but filling is safer.
};
...
<template>
    <div class="w-full h-screen bg-slate-100 overflow-hidden relative">
        ...
        <!-- Landing State -->
        <div v-if="!isMapActive" class="flex flex-col items-center justify-center h-full space-y-8 animate-fade-in">
            <div class="text-center space-y-4">
                <h1 class="text-5xl font-bold text-slate-800 tracking-tight">Solar MVP</h1>
                <p class="text-lg text-slate-600">Plan your solar installation in 3D</p>
            </div>

            <AtomButton @click="openAddressModal" variant="custom"
                ...
                Start Project
                <span
                    class="absolute inset-0 rounded-full ring-2 ring-white/20 group-hover:ring-4 group-hover:ring-indigo-300/30 transition-all"></span>
            </AtomButton>
        </div>
        ...
        <div v-if="showAddressModal"
            ...
            <div
                ...
                <div class="space-y-4 relative">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Address or City</label>
                        <div class="relative">
                            <AtomInput v-model="addressInput" @input="onInput" @keyup.enter="startMap"
                                placeholder="e.g. 1600 Amphitheatre Parkway" autofocus />
                            ...
                            <div v-if="suggestions.length > 0"
                                ...
                                <ul class="py-1">
                                    <li v-for="(item, index) in suggestions" :key="index"
                                        ...
                                        <span class="mt-1 text-slate-400 group-hover:text-indigo-500">📍</span>
                    ...
                    <div class="flex space-x-3 pt-2">
                        <AtomButton @click="cancelSearch" variant="secondary" custom-class="flex-1">
                            Cancel
                        </AtomButton>
                        <AtomButton @click="startMap" variant="primary" custom-class="flex-1">
                            Go to Map
                        </AtomButton>
                    </div>
                </div>
        ...
        <!-- Map State -->
        <div v-if="isMapActive" class="w-full h-full">
            <CesiumMap :initial-address="searchAddress">
                <AtomButton @click="backToHome" variant="glass" custom-class="flex items-center gap-2">
                    <span>←</span> Back
                </AtomButton>
            </CesiumMap>
        </div>
