...
const baseClasses =
  "font-medium transition-all focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed";
...
<template>
  <button
    :class="[baseClasses, variantClasses, customClass]"
    :disabled="disabled"
  >
    <IconLoading v-if="loading" width="20px" height="20px" color="white" />
    <slot v-else></slot>
  </button>
</template>
